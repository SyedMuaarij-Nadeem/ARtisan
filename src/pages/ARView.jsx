...
export default function ARView() {
  ...
  const [modelLoading, setModelLoading] = useState(true); // Asset Loading
  ...
  useEffect(() => {
    ...
    fetch(`${getBackend()}/api/model/${modelId}`)
      .then(r => r.ok ? r.json() : r.json().then(d => Promise.reject(d.error)))
      .then(d => {
        setModelUrl(d.modelUrl);
        setObjects([{ uid: modelId, name: 'Model', modelUrl: d.modelUrl, thumbnail: null }]);
      })
  ...
  const activeObj = objects[activeIdx];
  useEffect(() => {
    if (activeObj) setModelUrl(activeObj.modelUrl);
  }, [activeIdx]);
  ...
  const isReallyLoading = loading || !isMvReady || modelLoading;

  // Show internal loader if model data is still fetching or script/asset not ready
  if (isReallyLoading && !error) return (
    <div style={S.screen}>
      <style>{KF}</style>
      <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(circle at 50% 50%, #1A1A1A 0%, #0A0A0A 100%)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 24 }}>
        <div style={{ position: 'relative' }}>
          <div style={{ ...S.ring, width: 80, height: 80, borderWidth: 2, borderTopColor: '#8B5E3C' }} />
          <div style={{ position: 'absolute', inset: -10, border: '1px solid rgba(139,94,60,0.1)', borderRadius: '50%', animation: 'spin 2s linear infinite reverse' }} />
        </div>
        <div style={{ textAlign: 'center' }}>
          <p style={{ margin: 0, fontSize: 11, color: '#8B5E3C', letterSpacing: '0.6em', fontWeight: 900, textTransform: 'uppercase', animation: 'pulse 2s infinite' }}>Materializing Asset</p>
          <p style={{ margin: '8px 0 0', fontSize: 9, color: 'rgba(255,255,255,0.2)', letterSpacing: '0.2em', textTransform: 'uppercase' }}>Studio Syncing • Neural Buffer Active</p>
        </div>
      </div>
    </div>
  );
  ...
  if (error) return (
    <div style={S.screen}><style>{KF}</style>
      <div style={{ background:'rgba(239,68,68,0.05)', border:'1px solid rgba(239,68,68,0.2)', borderRadius:24, padding:'32px 24px', maxWidth:320, width:'90vw', textAlign:'center', backdropFilter:'blur(20px)' }}>
        <p style={{ margin:'0 0 8px', fontSize:16, fontWeight:600, color:'#fca5a5' }}>⚠️ Load failed</p>
        <p style={{ margin:'0 0 20px', fontSize:12, color:'rgba(255,255,255,0.5)', lineHeight:1.5 }}>{error}</p>
      ...
      <div style={{ ...S.topBar, opacity: isReallyLoading ? 0 : 1, transition: 'opacity 0.5s' }}>
        ...
        <div style={{ flex:1 }}>
          ...
          <p style={{ margin:'2px 0 0', fontSize:10, color:'rgba(255,255,255,0.4)' }}>
            {activeObj?.name || 'No model'}{objects.length > 1 ? ` · ${objects.length} objects` : ''}
          </p>
        </div>
